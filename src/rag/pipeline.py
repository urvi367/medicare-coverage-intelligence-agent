...
_GAP_SYSTEM = (
    ...
    "  (a) Count an abstract as on-topic if it studies THIS intervention for THIS condition "
    "(or the policy's clinical area), REGARDLESS of evidence tier — a review, guideline, or "
    "observational study still counts as on-topic (it is merely graded Weak later). The gate is "
    "about TOPICALITY, not study quality. Only if fewer than two abstracts are on-topic at all "
    "(after discarding name-collision abstracts) → Insufficient Evidence, stop. Do NOT fall back "
    "to Insufficient merely because the on-topic evidence is Weak/background tier or lacks an "
    "RCT/meta-analysis — weak on-topic evidence still yields a directional verdict.\n"
    ...
    "at least two on-topic abstracts. An abstract counts as on-topic regardless of its tier — "
    "do NOT exclude reviews, guidelines, or observational studies from this count; weak on-topic "
    "evidence is sufficient to render a directional verdict (graded Weak), not Insufficient.\n\n"
