...
Newest first. Phase 1 = Policy Q&A · Phase 2 = Gap Analysis · Phase 3 = LCD jurisdiction cascade.

| Version | Date | Changes |
|---------|------|---------|
| v3.4 | 2026-07-01 | **Diagnostic-test evidence anchoring (NCD + LCD).** Test policies have bare analyte titles ("HbA1c", "Magnesium"), so a plain PubMed search returns analyte-*as-therapy* studies → gap analysis over-credited them. Test policies now detected **statically** — NCD via `benefit_category`, LCD via CPT codes fetched through `lcd/related-documents → article/hcpc-code` (`data/lcd_diagnostic.json`, 293/695 LCDs) — and their search anchored on the test's own diagnostic accuracy. Regression-swept (NCD: 0 golden losses; LCD: 4 minor, 0 in eval). Diagnostic goldens re-labeled from the anchored evidence (4 flips Insufficient→Aligned). Also fixed a rate-limit bug in the LCD PubMed fetch (no throttle + swallowed 429s → global limiter + retry/backoff). **Gap-eval re-run to quantify is pending.** Honest limits: bare analytes with therapy-dominated literature (Magnesium) unfixed; MolDX proprietary assays retrieve ~0 (brand≠literature). |
| v3.3 | 2026-06-30 | **LCD faithfulness 0.655→0.843 — the gap was a metric artifact.** The low score was the *mandated jurisdiction disclaimer* (true, but absent from the LCD body → RAGAS flags it ungrounded on every answer; NCD has no such preamble). Disclaimer-excluded, grounding is **0.843 ≈ NCD 0.854** — LCD answers don't invent criteria. `judge_lcd` caches generated answers so re-scoring never regenerates. See §16.5. |
| v3.2 | 2026-06-30 | **LCD generation faithfulness measured** — reference-free RAGAS pass (`judge_lcd --faithfulness`, reuses `golden_lcd.json`). Raw `lcd_faithfulness` 0.655 (n=15); resolution is precise (0.94), generation the looser link (later shown a disclaimer artifact, v3.3). |
| v3.1 | 2026-06-28 | **Phase 3 complete — jurisdictional coverage as the NCD→LCD cascade (RAG).** Pivoted from title-matching to **body-based RAG**: LCDs re-fetched with full body, indexed (1,483 NCD + 10,668 LCD chunks) with boolean `mac_<key>` flags; `_hybrid_retrieve_lcd` is MAC-filtered. The cascade (`resolve_governing_policy`) is deterministic and shared by `answer()` + `gap_analysis()`; the agentic loop + `src/lcd/` package removed. LCD eval `lcd_precision` 0.94 / `lcd_recall` 0.783 (n=60); gate sweep validated `SILENT_GATE`=0.60, `LCD_GATE`=0.55. Index shipped via Git LFS. See §16. |
| v3.0 | 2026-06-27 | **Phase 3 kickoff — dynamic LCD lookup (branch `phase-3`).** Scoped to 5 MACs. Spike: CMS LCD list endpoint ignores server-side filters (~969 LCDs) and LCDs carry no state field → external state→MAC table. Step 1: `jurisdiction.py` (state→MAC, `ncd_disposition`). |
| v2.5 | 2026-06-27 | **Selection-tuning ruled out; disambiguation is the lever.** Two LLM-free backtests: `second_frac` sweep (recall ceiling 0.924 < old 0.942; recall↔contamination locked ~1:8) and scope-aware selection (of 26 runner-up=expected, only 3 hierarchically related) both fail to decouple recall from contamination — the signal is **semantic, not numeric**. Next lever: a narrow "which NCD governs?" disambiguation call. See §15.2.2. |
| v2.4 | 2026-06-25 | **Whole-NCD gap eval completed** (full 277). vs top-5-chunk baseline: `alignment_accuracy` 0.570→0.542, `kappa` 0.346→**0.433**, `faithfulness` 0.597→**0.705**, `ncd_recall` 0.942→0.888, Partial label_match 0.20→**0.33**. Verdict: a *trade, not a win* — the accuracy dip is **entirely** the capped NCD-selection's recall loss, not reasoning (kappa/faithfulness/Partial all up). See §15.2.2. |
| v2.3 | 2026-06-17 | **Primary-evidence re-ingest + whole-NCD context + full 277-record eval.** Re-fetched PubMed with a two-pass primary-evidence filter → **3,219 abstracts**, ~89% primary (was ~75% review); tier-aware grading (T1–T5). Re-labeled + Claude-adjudicated golden → **277 records**. Added `pmid_recall_retrieved`. **Whole-NCD context**: chunks now only *identify* the governing NCD (score-weighted vote, capped at 2), then feed its full text — fixes Partial gaps lost to chunk ranking (240.4 CPAP). A heuristic CED/boilerplate filter was prototyped and **rejected** (reliably regressed CPAP). |
| v2.2 | 2026-06-13 | **First gap eval + hardening.** 284-record reference fully Claude-adjudicated (67 overrides/24%). Fixed name-collision bug (topical join pulled same-name-different-intervention abstracts) via disambiguation in `_GAP_SYSTEM`/`_LABEL_PROMPT`; `LABEL_MODEL` flash-lite→flash. Two fixes on the 40-sample eval: topic-forward question regen (`ncd_recall` 0.75→0.925) and `pubmed_k` 8→12 (matches labeler budget) → `alignment_accuracy` 0.325→0.45, 0 catastrophic flips. |
| v2.1 | 2026-06-11 | **Phase 2 gap analysis implemented.** `gap_analysis()`: NCD-only hybrid retrieval joined to PubMed via **topical join** (`source_ncd_number == policy_number`; empty → "Insufficient Evidence", not unrelated abstracts). UI auto-routes policy vs gap via regex signal scoring. Independent-labeler gap eval scaffold (`generate_golden_gap.py` + `judge_gap.py`). Fixed PubMed date parser (ElementTree childless-element falsy → 99% blank years). |
| v2.0 | 2026-06-10 | **Hybrid BM25 + dense retrieval** with RRF (k=60) — BM25 fixes numeric-threshold failures ("55 mmHg"). Chroma object cached in `_get_db()`. UI feedback buttons. Deployed to Streamlit Community Cloud. |
| v1.9 | 2026-06-10 | **Phase 2 scaffold**: `fetch_pubmed.py` (NCBI E-utilities, per-NCD search, PMID dedup) + `pubmed_indexer.py` (separate `pubmed_evidence` collection). |
| v1.8 | 2026-06-10 | **First clean Phase-1 baseline.** Root-caused persistent HTML entities: CMS double-escapes (`&amp;gt;`) → `_strip_html` unescapes to fixed point (tag regex spares `< 80 mm Hg`). Fixed `build_index` append-on-rebuild (Chroma fresh IDs → 4× duplication, 8473 chunks) with a wipe-before-rebuild. LCD addendum now fires only when the top doc is an LCD. |
| v1.7 | 2026-06-10 | Widen retrieval to k=10 / threshold=0.65 (reranker selects 5-of-10). Config-derived answer-cache path (no stale reuse). Judge → `gemini-2.5-flash`; add `policy_recall`. |
| v1.6 | 2026-06-09 | Add title-prepend + synonym expansion (50+ clinical↔CMS pairs) per chunk at index time (vocabulary bridge). Context Precision target met (0.832). |
| v1.5 | 2026-06-09 | Add cross-encoder reranker (bge-reranker-base). Fix NCD-only eval bug (legacy LCD cache entries contaminating scores). |
| v1.2–v1.4 | 2026-06-07/08 | Switch to gemini-2.5-flash; persistent answer cache; rate-limit retry via API `retryDelay`. Fix AnswerRelevancy NaN (BAAI embeddings + `bypass_n=True`); paid API tier; per-sample score logging. |
| v1.0–v1.1 | 2026-03/05 | Initial PRD (Phase 1, Groq/llama); updated to reflect live implementation. |
...
**Thesis.** LCDs are jurisdictional — one MAC contractor per region, the *same* service covered differently across MACs, ~969 final LCDs. Coverage must therefore resolve **national-first, then jurisdictional**: an NCD governs nationally; absent one (or when it defers to local contractors), the beneficiary's **MAC LCD** applies.

### 16.1 Final architecture — the NCD→LCD cascade (RAG)

`resolve_governing_policy(question, state)` is the shared resolution step both `answer()` (Policy Q&A) and `gap_analysis()` call — **not a separate path**:

```
question (+ optional state)
  → retrieve + rerank NCDs;  NCD governs (relevance gate ≥ SILENT_GATE + ncd_disposition)?
       ├─ governs → return the NCD                                         ✅
       └─ silent / defers-to-MAC
            → state → MAC  (deterministic table; ask the user if state unknown)
                 ├─ out-of-scope MAC → none ("unsupported jurisdiction")
                 └─ in-scope MAC → MAC-filtered RAG over indexed LCD bodies
                      ├─ top LCD ≥ LCD_GATE → return the LCD
                      └─ else → none ("no determination → contractor discretion")
```

The cascade is **deterministic** (relevance gates + defer-marker + state→MAC); only the final answer/gap reasoning is generative.

### 16.2 Key decisions (what was tried and rejected)

- **Live tool-use → RAG.** The initial design fetched LCDs live and matched them to questions by **title** (`lcd_lookup` + a Gemini function-calling orchestrator). It failed: questions don't carry LCD titles ("panniculectomy" vs an LCD titled "Plastic Surgery"). **Replaced with body-based RAG** — LCDs are indexed like NCDs and retrieved by content. The live `lcd_lookup`, orchestrator, and the separate `src/lcd/` package were removed.
- **Agentic loop → deterministic cascade.** The NCD→LCD flow is deterministic, so the function-calling loop was over-engineering. The cascade lives in `pipeline.py` beside the NCD logic.
- **Spike finding.** The CMS LCD list endpoint ignores all server-side filters (returns ~969 LCDs) and LCDs carry no state field — only `contractor_name_type`. Hence the external state→MAC table and the local body index.

### 16.3 LCD indexing & evidence

- **Indexed like NCDs:** the indications/limitations **body** is re-fetched with full text (the old snapshot was titles-only — the license token had failed), chunked with title-prepend + synonym expansion, and tagged with **boolean `mac_<key>` flags** (one doc per LCD even when shared across MACs — no duplication; Chroma metadata is scalar). Retired LCDs excluded. `cms_coverage`: **1,483 NCD + 10,668 LCD chunks**.
- **LCD retrieval** = `_hybrid_retrieve_lcd` — body-based, filtered to the beneficiary's MAC flag, no dense-score threshold (short clinical queries fall below the 0.65 cosine cut), cross-encoder gated. A regex `_denoise` strips coverage-question boilerplate so the query isn't drowned by shared LCD language.
- **LCD gap evidence:** `fetch_lcd_evidence` searches PubMed per LCD topic (by title, with a diagnostic-test anchor — see §16.7), tags `source_lcd_number`; `_pubmed_for_lcd` topical-joins on it. `pubmed_evidence`: **~7,179 abstracts (NCD + LCD)**, tier-graded. *Limits:* broad LCD titles ("Plastic Surgery") and MolDX proprietary-assay brand names fetch no focused evidence → those LCDs return Insufficient (accepted; no live fallback, per product call).

### 16.4 Scope & jurisdiction

Five MACs end-to-end — **Noridian, CGS, WPS, Palmetto, National Government Services** (best-represented). First Coast / Novitas states resolve to *unsupported jurisdiction* (reported, never guessed). The UI asks for the state when a jurisdictional question lacks one (multi-turn).

### 16.5 Evaluation & gate tuning

- **LCD cascade eval** (`generate_golden_lcd.py` + `judge_lcd.py`; Groq writes a lay question per sampled LCD, 12/MAC = 60; the judge runs the cascade, no API cost): **`lcd_precision` 0.94, `lcd_recall` 0.783** (disposition lcd 50 / ncd 7 / none 3). The 7 "ncd" are NCD-governed services the cascade correctly intercepts → LCD-applicable recall **0.887**.
- **LCD generation faithfulness** (`judge_lcd --faithfulness`; reference-free RAGAS grounding of the answer in the LCD body, n=15 of LCD-resolved; reuses `golden_lcd.json`, no labels): **`lcd_faithfulness` 0.843 — on par with the NCD policy answer (0.854)**; LCD answers do not invent criteria. The raw score is 0.646, but the gap is a **measurement artifact, not hallucination**: the LCD answer prompt *mandates* an opening jurisdiction disclaimer ("based on the {mac} LCD … coverage may differ in other regions") that is true but absent from the LCD body, so RAGAS flags it ungrounded on every answer (~0.2 tax; NCD answers have no such preamble). Disclaimer-excluded (`strip_disclaimer`) the score jumps 0.646→0.843. **This also confirmed the decouple+extractive-prompt refactor wasn't the lever — there was no grounding problem; the score was disclaimer-bound.** Answers are cached (`lcd_faith_answers.json`) so re-scoring never regenerates.
- **Gate sweep** (`scripts/sweep_lcd_gates.py`, caches scores once → all combos, no API): `SILENT_GATE`=0.60 confirmed (stable [0.58, 0.65]; 0.55 over-fires NCD governance) and `LCD_GATE`=0.55 confirmed precision-optimal (0.94; 0.50 buys ~2pts recall but turns "verify" into wrong-LCD). The two soft gates are now eval-validated, not guessed.
- **Four thresholds**, distinct stages/score-types: dense-cosine **0.65** (retrieval admission, bi-encoder; Phase-1 RAGAS-tuned) · `second_frac` **0.70** (1-vs-2 NCDs, score ratio; Phase-2 swept) · `SILENT_GATE` **0.60** (NCD-governs gate, cross-encoder sigmoid) · `LCD_GATE` **0.55** (LCD-governs gate, cross-encoder sigmoid).

### 16.6 Structure & deployment

Phase 3 was **folded into existing files** (no `src/lcd/` package): jurisdiction routing + the cascade live in `src/rag/pipeline.py`; ingestion in `fetch.py` / `fetch_pubmed.py`. The vector index (~184MB) is committed via **Git LFS** so Streamlit Cloud deploys without a rebuild.

**Open (future):** per-service LCD evidence (the broad-title gap), MolDX brand→gene-signature query expansion, and system-wide lay-query/synonym normalization (affects NCDs too).

### 16.7 Diagnostic-test evidence anchoring (v3.4)

Diagnostic/lab-test policies carry bare analyte titles ("HbA1c", "Magnesium"), so a plain per-topic PubMed search returns *analyte-as-therapy* studies (magnesium supplementation) — right topic, wrong subject — which made gap analysis over-credit them (labeler correctly said Insufficient; pipeline said Aligned).

- **Static detection, no LLM.** NCD test policies are flagged by the CMS `benefit_category` field (every comma-part must start "Diagnostic", so mixed drug/therapy policies stay on the normal path). LCDs have no such field, so they're classified by **CPT codes** fetched via the associated Billing & Coding article: `lcd/related-documents → r_article_id → article/hcpc-code`, diagnostic iff every numeric CPT is in a diagnostic range (imaging/lab/cardio/pulm/neuro) with no drug (J/Q) code → `data/lcd_diagnostic.json` (293/695 LCDs; 0 golden mismatches).
- **Anchored search.** Flagged policies search the test's own performance (`_DIAGNOSTIC_ANCHOR`: sensitivity/specificity/accuracy/screening/measurement), kept on the backfill pass so a shortfall isn't filled with therapy.
- **Regression-checked before shipping.** NCD: 93 flagged, 0 of the 72 in the golden lose evidence. LCD: true anchor-caused regression is 4 LCDs (minor, 0 in eval); the 149 that retrieve <2 are pre-existing MolDX brand-name breakage, unchanged by the anchor.
- **Goldens re-labeled** from the anchored evidence (8 NCD + 7 LCD diagnostic records; 4 flips Insufficient→Aligned, e.g. Sleep Testing, HbA1c). **Gap-eval re-run to quantify the effect is pending** (paid regeneration deferred).
- **Honest limits.** Bare analytes whose PubMed is therapy-dominated (Magnesium) stay Insufficient — anchoring can't manufacture test-accuracy literature; proprietary MolDX assays retrieve ~0 because brand names don't match the literature (separate query-expansion task).

---

*Medicare Coverage Intelligence Platform · PRD v3.4 · All data sources public · Last updated 2026-07-01*
