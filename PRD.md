...
- **LCD generation faithfulness** (`judge_lcd --faithfulness`; reference-free RAGAS grounding of the answer in the LCD body, n=15 of LCD-resolved): **`lcd_faithfulness` 0.655** — below the NCD policy answer (0.854). LCD bodies are long and boilerplate-heavy, so answers extrapolate criteria more; a real quality gap (the resolution is precise, the generation less grounded). Reusing the existing `golden_lcd.json` — faithfulness is reference-free, so no labels needed.
